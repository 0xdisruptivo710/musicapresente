  ...
  return (
    <main className="relative min-h-dvh overflow-hidden bg-black">
      ...
      <div className="relative flex min-h-dvh flex-col items-center justify-center px-6 py-16 text-center">
        ...
        {data.message ? (
          <p className="mt-5 max-w-lg border-l-2 border-violet-400 pl-4 text-left text-base italic leading-relaxed text-zinc-100 sm:text-lg">
            “{data.message}”
          </p>
        ) : null}
        ...
        {!started ? (
          <button
            ...
            className="mt-9 rounded-2xl px-7 py-3.5 text-sm font-semibold text-white shadow-lg shadow-violet-900/40 transition hover:opacity-95"
          >
            ...
            {photos.map((photo, i) => (
              <span
                ...
                className={`h-1.5 rounded-full transition-all ${
                  i === idx ? "w-6 bg-violet-400" : "w-1.5 bg-white/30"
                }`}
              />
            ))}
          </div>
