...
export function OfferFaq() {
  ...
  return (
    <section className="px-4 py-12 sm:px-6">
      <h2 className="mb-6 text-center font-serif text-2xl font-bold text-ink sm:text-3xl">
      ...
      <div className="mx-auto flex max-w-2xl flex-col gap-3">
        {FAQS.map((faq, i) => {
          const isOpen = open === i;
          return (
            <div key={faq.q} className="overflow-hidden rounded-2xl border border-hair bg-white">
              <button
                type="button"
              ...
              >
                {faq.q}
                <span className={`shrink-0 text-lg text-brand transition-transform ${isOpen ? "rotate-45" : ""}`}>
                  +
                </span>
              </button>
